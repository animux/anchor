import { CohortOverviewDashboard } from "@/components/dashboard/cohort-overview-dashboard"
...
export default async function DashboardPage() {
  ...
  const [{ data: studentsData }, { data: summaryData }, { data: groupRows }] =
    await Promise.all([
  ...
  const initialStudents: SubmissionStudent[] = studentRows
    .filter((row) => row.overall_rag === "amber" || row.overall_rag === "red")
    .map((row) => ({
      id: row.id,
  ...
  return (
    <CohortOverviewDashboard
      initialStudents={initialStudents}
      initialTotalCount={initialStudents.length}
